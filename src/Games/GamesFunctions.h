...
void DrawTextWithRectangle2(const wchar_t* text, float x, float y, int* color1, bool underline)
{
	if (underline)
	{
		int RAINBOW[3] = { RainbowRED1, RainbowGREEN1, RainbowBLUE1 };
		...
		DrawRectangle(x, y, width + 4, 11, color1);
		DrawRectangle(x, y + 11, width + 4, 2, RAINBOW);
		DrawText(text, x + 2, y + 2, color(MC_Color::LightPurple));
	}
	else
	{
		int width = Font_width(text);
...
void DrawTextDebugPlayer(const wchar_t* text, float x, float y, int* bgColor, int* textColor, bool underline)
{
	if (underline)
	{
		int RAINBOW[3] = { RainbowRED1, RainbowGREEN1, RainbowBLUE1 };
...
void DrawTextCharWithRectangle2(char* text, float x, float y, int* textColor, bool underline)
{
	if (underline)
	{
		int RAINBOW[3] = { RainbowRED1, RainbowGREEN1, RainbowBLUE1 };
...
void GetTeleportXYZ()
{
	DrawRectangle(447 + MenuX, 52 + MenuY, 50, 33, MC_Color::BlackTheme);

	char option4[0x100];
	wchar_t woption4[0x100];
	_sys_memset(option4, 0, 0x100);
	_sys_memset(woption4, 0, 0x200);
	_sys_snprintf(option4, 0x100, "X: %i   ", (CoordX));
	StringToWideCharacter(woption4, option4, _sys_strlen(option4));
	DrawTextC(option4, 450 + MenuX, 55 + MenuY, color(MC_Color::White));

	char option8[0x100];
	wchar_t woption8[0x100];
	_sys_memset(option8, 0, 0x100);
	_sys_memset(woption8, 0, 0x200);
	_sys_snprintf(option8, 0x100, "Y: %i   ", (CoordY));
	StringToWideCharacter(woption8, option8, _sys_strlen(option8));
	DrawTextC(option8, 450 + MenuX, 65 + MenuY, color(MC_Color::White));

	char option16[0x100];
	wchar_t woption16[0x100];
	_sys_memset(option16, 0, 0x100);
	_sys_memset(woption16, 0, 0x200);
	_sys_snprintf(option16, 0x100, "Z: %i   ", (CoordZ));
	StringToWideCharacter(woption16, option16, _sys_strlen(option16));
	DrawTextC(option16, 450 + MenuX, 75 + MenuY, color(MC_Color::White));
}
